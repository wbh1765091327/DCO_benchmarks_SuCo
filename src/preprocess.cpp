...
void load_data(float ** &dataset, char * dataset_path, long int dataset_size, int data_dimensionality) {
    ...
    for (int i = 0; i < dataset_size; i++) {
        ...
        int dim;
        fread_return = fread(&dim, sizeof(int), 1, ifile_dataset);
        
        // 验证维度是否匹配
        if (dim != data_dimensionality) {
            cout << "Error: Dimension mismatch! Expected " << data_dimensionality 
                 << ", but got " << dim << " in vector " << i << endl;
            exit(-1);
        }
...
void load_query(float ** &querypoints, char * query_path, int query_size, int data_dimensionality) {
    ...
    for (int i = 0; i < query_size; i++) {
        ...
        int dim;
        fread_return = fread(&dim, sizeof(int), 1, ifile_query);
        
        // 验证维度是否匹配
        if (dim != data_dimensionality) {
            cout << "Error: Dimension mismatch! Expected " << data_dimensionality 
                 << ", but got " << dim << " in query " << i << endl;
            exit(-1);
        }
...
void load_groundtruth(long int ** &gt, char * groundtruth_path, int query_size, int k_size) {
    ...
    for (int i = 0; i < query_size; i++) {
        ...
        // 读取 .ivecs 格式：先读取维度，再读取数据
        int dim;
        fread_return = fread(&dim, sizeof(int), 1, ifile_groundtruth);
        
        // 读取 int 类型数据，然后转换为 long int
        int *temp_gt = new int[k_size];
        fread_return = fread(temp_gt, sizeof(int), k_size, ifile_groundtruth);
        
        // 转换为 long int
        for (int j = 0; j < k_size; j++) {
            gt[i][j] = (long int)temp_gt[j];
        }
        delete[] temp_gt;
    }
